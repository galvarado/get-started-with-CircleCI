...
if [ ! -z "$PIPE_IDS" ]; then
  ...
  do
    curl --header "Circle-Token: $CIRCLE_TOKEN" --request GET "https://circleci.com/api/v2/pipeline/${PIPE_ID}/workflow"|jq -r --arg WF_NAME "${WF_NAME}" '.items[]|select(.status == "running") | select(.name == $WF_NAME) | .id' >> WF_to_cancel.txt
  done
fi
...
if [ -s WF_to_cancel.txt ]; then
  ...
  while read WF_ID;
    do
      curl --header "Circle-Token: $CIRCLE_TOKEN" --request POST https://circleci.com/api/v2/workflow/$WF_ID/cancel
